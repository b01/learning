...
Manually build `etcd` CA, keys, and certificates.

1. Set up a working directory and environment by logging into the
   ...
   envsubst < /vagrant/openssl-etcd.conf.tmpl > ${ETCD_CERT_CONF}
   envsubst < /vagrant/openssl-control-plane.conf.tmpl > openssl-control-plane.conf
   ```
2. Make a Certificate Authority for `ectd` server that can be used to generate
   ...
   sudo rm -rf ${etcd_cert_dir}/ca.srl
   ```
...
sudo openssl x509 -req -days ${CERT_DAYS} -in "${i}.csr" \
 ...
 -out "${cert_dir}/${i}.crt"
```
NOTE: We are signing the certificate with the `ectd` server Certificate
Authority, and not the Kubernetes Certificate Authority. Which will allow the
`kube-apiserver` to authenticate with the `etcd` server to manage data.
...
1. Generate a TLS key-pairs for control-plane components that will be
   embedded in the kubeconfigs at `/etc/kubernetes`.
   ...
   # remove the openssl serial no. count increment file.
   sudo rm -rf ${cert_dir}/ca.srl
   ```

## Generate kubelet Certificates

Deprecated, we want to use TLS Bootstrapping, see next step

For each node, we're going to need a certificate for the `kubelet` service to
authenticate with the `kube-apiserver`. Each certificate will be unique, as
the hostname and IP addresses of the node will be embedded as subject
alternative names. In addition, these certificates will also double as the
serving certificate when clients need to connect to the `kubelet`, such as when
the `kube-apiserver` want to fetch metrics. See
[Kubelet's server and client certificates].

1. Copy the OpenSSL config file `openssl-kubelet.conf` to the
   `control-plane-01` nodes' `~/certs` directory.
2. Build a `machines.txt` containing the following:

   ```shell
   cat <<TXT | tee machines.txt
   control-plane-01 2000:192:168:56::11 192.168.56.11
   worker-01        2000:192:168:56::21 192.168.56.21
   worker-02        2000:192:168:56::22 192.168.56.22
   TXT
   ```

   NOTE: You would normally look for this using a tool like `ip a`, then noting
   the IPs on the network interface you planned to use for your cluster network.
   Again, checking on each node.
3. Generate a key and certificate (signed by the Kubernetes CA) for each node's
   `kubelet` to use.
   ```shell
   cd ~/certs
   KUBELET_CONF="openssl-kubelet.conf"

   while read NODE_NAME NODE_IPV6 NODE_IPV4
   do
       # generate a key
