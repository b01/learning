...
1. Clone this repository locally
   `git clone https://github.com/b01/learning.git`.
2. Open a terminal to this repository and change to the `samples` directory:
   ```shell
   cd learning\kubernetes\samples
   vagrant up
   ```
   NOTE: Each machines private IP is hard-coded in the Vagrantfile so that they
   are predictable for the sake of these instructions. We will focus on using
   IPv6 addresses as well. The machines are:
   * `control-plane-01`: `fd00:192:168:56::11`,
   * `worker-01`: `fd00:192:168:56::21`, and
   * `worker-01`: `fd00:192:168:56::22`.
   ...
   the hostname of the VMs, run `vagrant status`. Or type `hostname` after
...
NOTE: Currently, Kubernetes v1.33+ defaults to `systemd`. In the future it may
autodetect it.
...
2. Swap configuration
   ...
   3. To persist this change on reboots on Ubuntu edit `sudo vi /etc/fstab` and
      ...
      you can also use `sed` to quickly make this change, like so:

      ```shell
      sudo sed -i 's/\(\/swap.img.*\)$/#\1/' /etc/fstab
      ```
   4. Then save the file, with vim, you type `:`, then `x`, then hit enter.
3. Configure kernel to forward IP for IPv6 and IPv4:
   ...
   3. Optionally [Enable IPv6 packet forwarding]:
      1. check if it is turned on with `sudo sysctl net.ipv6.conf.all.forwarding`
      2. If not, then run:
...
4. CNI plugins may use `iptables` and the `bridge-netfilter` architecture of
   the Linux kernel. So we need to load the [br_netfilter] kernel module and
   enable IP packet filter for Linux based bridge networks.
...
5. Gather each machines private IP. You'll want to run `ip address` on each
   ...
   If you are using the Vagrantfile that came with this guide, then the IPs are
   actually hard-coded in the [Vagrantfile] to be. However, be mindful that will
   not be the case if you chose another setup, save VMs in AWS. That will
   require you to know what you are looking for.

   On each VM we'll edit the `/etc/hosts` file so that each machine can find the
   other by name. This will allow us to communicate with each node by hostname
   instead of IP address, which should reduce errors when copying files around.
   ...
   nodes Internet access. The second adapter on each node is the adapter for
   our k8s network.

   On AWS EC2 instances there may be only 1 NIC, the first one `ens0`.
6. Ensure the SSH daemon on all nodes allow keyboard-interactive
   authentication:
...
7. Back on the `control-plane-01` node make an SSH key, then use `ssh-copy-id`
   ...
   NOTE: You can just hit enter and leave the password blank if you're just
   practicing.
...
[Install containerd]: /kubernetes/003.2-install-containerd.md#install-containerd
