...
Kube-router need to access kubernetes API server to get information on pods,
services, endpoints, network policies etc.

1. Make a kube-config for `kube-router` to give it access to the `kube-apiserver`
   ```shell
...
3. For pod-to-pod network connectivity set the `controller manager` flags to
   ...
   cluster CIDRs, such as `--cluster-cidr=fd12:3456:789a:1000::/56,10.244.0.0/16`.
