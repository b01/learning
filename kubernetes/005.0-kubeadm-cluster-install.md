...
* To enable dual-stack and have `kubeadm` assign both IPv6/IPv4 addresses to the
  kubelet; with IPv6 preferred. Then you MUST set `advertiseAddress` to the
  nodes adapter's IPv6 address. As an IPv4 will leave it in IPv4 mode only.
  ```yaml
  ---
  ...
  ```
  You will also be required to list the IPv6 CIDR for Pod and Service CIDRs.
