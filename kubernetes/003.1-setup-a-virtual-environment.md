...
2. Open a terminal to this repository and change to the `samples` directory:
   ...
   `control-plane-01=192.168.56.11`, `worker-01=192.168.56.21`, and
   `worker-01=192.168.56.22`. If you increase the number of nodes by setting
   the environment variable `NODE_WORKERS` to a number greater than 2, then
   there IPs are sequential. There is also IPv6 support, so you can do a
   "DualStack" cluster. You'll have IPv6 `2000:192:168:56::11` for the
   `control-plane-01`, `2000:192:168:56::21` for `worker-01`, and
   `2000:192:168:56::22` for `worker-02`.
...
3. Optionally [Configure SSH From A JumpBox].
...
5. Gather each machines private IP. You'll want to run `ip address` on each
   ...
   nodes internets access, but also has the same Mac and IP address on each
   node. The second adapter on each node is the adapter for our K8s network,
   with each having different Mac and IP addresses, a requirement for a k8s
   network.
   You may not see a second adapter in your own custom setup. So use the
   adapter on each node that is on the same network with unique Macs and IPs.
   On an AWS EC2 instance the first one `ens0` or whatever should be fine.
...
[Install containerd]: /kubernetes/003.3-install-containerd.md#install-containerd
