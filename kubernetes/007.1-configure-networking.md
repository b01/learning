...
1. Another alternative, install `kube-router` addon, which also provides
   ...
   2. Make a `kube-router` manifest, this will also replace `kube-proxy`.
      ...
      5. Perform `kube-proxy` cleanup
         ```shell
         sudo ctr images pull registry.k8s.io/kube-proxy:v1.34.0
         sudo ctr run --rm --privileged --net-host --mount type=bind,src=/lib/modules,dst=/lib/modules,options=rbind:ro \
             registry.k8s.io/kube-proxy:v1.34.0 kube-proxy-cleanup kube-proxy --cleanup
         ```

         NOTE: Here are a list of all [configuration options] for kube-router.
2. Test that DNS resolution for the cluster is working:
   1. Launch a Pod with DNS tools:
      ```shell
      kubectl run --restart=Never --image=infoblox/dnstools:latest dnstools -- ["while true; do echo done"]
      ```
      if you already added nodes, then:
      ```shell
      kubectl run -it --rm --restart=Never --image=infoblox/dnstools:latest dnstools
      ```
   2. Edit the po so that it has a tolerance effect of `NoSchedule` and
      operator of `Exists`
      ```shell
      kubectl edit po dnstools
      ```
   3. Query the DNS server:
      ```shell
      kubectl exec -it dnstools -- host kubernetes
      ```
   4. Now type `exit` and it should remove the  `dnstools` Pod since we used
      `--rm` flag.
