...
7. As an alternative to Flannel if you want to get practice with network
   ...
   3. Modify the Antrea configuration to use the CIDRs previously chosen:
      ...
      enabled.
8. Let install `kube-router` addon to replace `kube-proxy` and as our CNI of
   choice. It provides Network Policy features. `kube-router` needs to access
   ...
   2. Generate a certificate for access the `kube-apiserver`:
      ...
      sudo openssl x509 -req -days ${CERT_DAYS} -in "${i}.csr" \
       ...
       -out "${i}.crt"
      ```
   3. Make a kubeconfig for `kube-router` Static Pod to mount and gain access
   ...
   7. Fill in placeholders then apply the manifest:
      ...
      ```
9. Test that DNS resolution for the cluster is working:
