...
export IPV4=${IP4}
export IPV6=${IP6}
envsubst < openssl-etcd.conf.tmpl > openssl-etcd.conf
...
2. Build a `machines.txt` containing the following:
   ```shell
   cat <<TXT | sudo machines.txt
   control-plane-01 <ipv4-address> <ipv6-address>
   worker-01        <ipv4-address> <ipv6-address>
   worker-02        <ipv4-address> <ipv6-address>
   TXT
   ```
