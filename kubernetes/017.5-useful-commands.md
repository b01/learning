...
```shell
ETC_SERVER="192.3.76.3"
ETCDCTL_API=3 etcdctl \
    --endpoints=https://${ETC_SERVER}:2379 \
...
Review kubeadm config, you can look at this config file with:
...
NOTE: While not fool-proof as there could be more than 1, or old files.

Get the CIDR of the nodes in the cluster or Pods: `ip addr show`
NOTE: Look for the interface that the internal IP of the control plane node
      is tied to. For Pods look for the CNI plugin.

Get CIDR of IP addresses configured for Pods, once you know which CNI plugin is
used to configure then review the logs and look for the value of "ipalloc-range":
...
Get the __type__ of proxy the kube-proxy will use at runtime (since it can be
given multiple options at initialization):
```shell
...
List a node's labels using `kubectl get nodes --show-labels`.
