...
1. Another alternative, install `kube-router` addon, which also provides
   ...
   1. When run as `DaemonSet`, the container image is prepackaged with
      ...
      ```
   2. Make a `kube-router` manifest, you have 2 options.
      1. Use existing `kube-proxy`
         1. Download the manifest to work with `kube-proxy`
            ```shell
      ...
      2. Replace `kube-proxy`
         1. Download the manifest with all features enabled
            ...
            ```
         2. Edit the manifest to add the option `--router-id=generate` to the
            arguments for the `kube-router` container.
...
[Add Workers]: /kubernetes/005.2-join-workers.md
