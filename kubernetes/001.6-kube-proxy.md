...
It can use the operating system packet filtering layer if there is one, and it's
...
itself (like Antrea), and providing equivalent behavior to `kube-proxy`, then
you can enable that feature and remove `kube-proxy` from your cluster.
...
nodes will not be able to join the cluster, deploy a DNS, or a CNI addon
until kube-proxy or a similar replacement is running.

Next: [Cluster Installation Overview]
---
