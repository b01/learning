...
3. Add auto complete for `kubectl` and the shortcut `k`
   ...
   ```
4. In addition, we want to be able to debug cluster components, so install
   `critctl` tool. Download and install into `/usr/local/bin` like so:
   ...
   crictl --version
   ```
   NOTE: You can use this tool to check the status and logs of static Pods.
5. Make a config for `critctl`
   ```shell
...
Next: [Manual Cluster Install] or [kubeadm Cluster Install] or [Install ECR Credential Provider]
...
[Install ECR Credential Provider]: /kubernetes/006.3-install-ecr-credential-provider.md
