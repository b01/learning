...
1. Make a deployment named `nginx-ex-dp-1`, based on the latest version of Nginx,
   ...
   "local-storage". Give any other resource objects that you need to make the
   suffix `-ex-dp-1`.

   **Requirements**
   ```text
   deployment name: `nginx-ex-dp-1`
   ...
   1. Once the containers have successfully deployed; place a file on each
      worker at `/tmp/make-a-deployment-index.html` with the HTML content
...
2. Now that you have a deployment, make a [Service] using NodePort named
   `nginx-ex-dp-1-svc`.
   1. Retrieve the node port assigned to the `nginx-ex-dp-1-svc` service:
      ```bash
      NODE_PORT=$(kubectl get svc nginx-ex-dp-1-svc \
