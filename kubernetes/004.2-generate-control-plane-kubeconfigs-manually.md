...
7. Generate a bootstrap kubeconfig for the control-plane-01 node's`kubelet`
   to use:
   1. Make a [Token authentication file] for the `kubelet` to access the
      `kube-apiserver` as a bootstrapper when it starts:
   ...
   2. Make a bootstrap kubeconfig file:
      ...
      sudo kubectl config use-context bootstrap \
        --kubeconfig=/var/lib/kubelet/bootstrap.conf
      ```
