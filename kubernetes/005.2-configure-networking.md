...
2. Install a CNI addon, you have several choices:
    ...
    3. Another alternative, install `kube-router` addon, which also provides
       ...
       `kube-router` needs to access the kubernetes API server to get
       information on pods, services, endpoints, network policies etc.
        ...
        2. We'll generate a certificate for `kube-router` so it can access the
           ...
           cd ~/certs
           getIps "ens5"
           wget https://raw.githubusercontent.com/b01/learning/refs/heads/main/kubernetes/samples/openssl-kube-router.conf.tmpl
           ...
           CLUSTER_ENDPOINT=https://control-plane:6443
           i="kube-router"
           ...
           # sign the CSR using the cluster CA key.
        ...
        4. Make a `kube-router` manifest:
           ...
           wget -O kube-router.yaml.tmpl https://raw.githubusercontent.com/cloudnativelabs/kube-router/master/daemonset/generic-kuberouter-all-features.yaml
           ```
        5. Edit the `kube-router.yaml.tmpl` and add a user subject to the
           ...
           ```
        6. Fill in placeholders then apply the manifest:
           ```shell
           CLUSTERCIDR=2001:db8:42:1::/112,10.96.0.0/16 \
           APISERVER=https://control-plane:6443 \
           sh -c 'cat kube-router.yaml.tmpl | \
           sed -e "s;%APISERVER%;$APISERVER;g" -e "s;%CLUSTERCIDR%;$CLUSTERCIDR;g"' | tee kube-router.yaml
