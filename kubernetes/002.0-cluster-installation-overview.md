...
`kubeadm`. Each has its own benefits. Also, these guides default to
installing "DualStack" clusters. However, you will also learn how to make a
only IPv6 or only IPv4 from comprehending this same information.
...
3. A network addon (CNI) that supports dual stack.
4. To pass in IPv6 and IPv4 CIDRs to the following components:
...
`--advertise-address` option to an IPv6 address, then the cluster will be IPv6
Primary, so it will default to using IPv6 addresses.
...
4. [Generate Static Pod Manifests] for components:
   ...
   4. `kube-scheduler`
5. Prepare [TLS bootstrapping] before starting the `kubelet` service.
6. [Initialize the Control Plane] by starting the `kubelet` service on the
   `control-plane-01` node.
7. [Configure Networking] by deploying a CNI network addon.
8. [Add Workers] nodes to the cluster.
