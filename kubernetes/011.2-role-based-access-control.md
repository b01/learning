...
To see resources and their corresponding apiGroup, run:
```shell
kubectl api-resources -o wide
```
The "APIVERSION" column mentions the apiGroup name you can use. Just drop the
`/v1<beta1>`. for example:
```shell
...
* `pods` is on the core apiGroup
* `deployments` in in the `apps` apiGroup
